...
class TwoArmTrajectory:
    ...
    def ConvertToCSV(self, target_filepath):
        ...
        csv_string = ""
        for state in self.states:
            line_str = self.__array_to_csv__(state.left.actual) + "," + self.__array_to_csv__(state.right.actual) + "," + state.left.secs + "," + state.left.nsecs + "\n"
...
if __name__ == '__main__':
    print "Converting all files in CWD to CSV..."
    files = os.listdir("./")
    count = 0
    for filename in files:
        if (".xml" in filename):
            print "Converting file: " + filename
            traj = TwoArmTrajectory(filename)
            traj.ConvertToCSV(string.replace(filename, ".xml", ".csv"))
            count += 1
    print "...conversion of " + str(count) + " files complete"
